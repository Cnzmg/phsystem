...
var localURL = "http://mapi.cbcoffee.cn/phsystem/";
...
jzm.getQueryToken = function (){
  ...
  .done(function(reg){
    ...
    //判断reg.statusCode.status里面的返回状态码是否为statusCode里面的其中一个编码如果是就进入错误信息方法否则对获取到的userToken和userId进行加密后存进sessionStorage
    // stateCode.test(reg.statusCode.status) ? jzm.Error() : sessionStorage.setItem("token",JSON.stringify({token:jzm.compilestr(reg.idKey),id:jzm.compilestr(reg.userId.toString()),uri:/*jzm.compilestr(window.location.pathname.split('?')[0])*/jzm.compilestr('/phsystem/index.html')}));
    stateCode.test(reg.statusCode.status) ? jzm.Error() : sessionStorage.setItem("token",JSON.stringify({idKey:reg.idKey,uri:jzm.compilestr(window.location.pathname.split('?')[0]),loginType:reg.loginType}));
...
jzm.Error = function (err){   //错误信息
    ...
    // mBox.open({
	  ...
	  //   time: 3 //3秒后自动关闭
    // });
    err.statusCode.status == 4444 ? document.write(err.statusCode.msg) : window.location.href = "./fake_html/fake-index.html?uri=" + encodeURI(window.location.href.split('?')[0]);
};
...
jzm.uncompilestr = function(code){
  // console.log(code)
    ...
    // console.log('长度：'+String.fromCharCode(code.length))
