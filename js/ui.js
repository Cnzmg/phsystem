...
function dateSearch(num) {
	...
	var time = setInterval(function () {
		// console.log(111)
		var startTime = $('#startTime').html() || false;
		var endTime = $('#endTime').html() || false;
	...
	if (target.nodeName.toLowerCase() == 'button') {
		...
		switch (target.className) {
			...
			case 'this-m':
				...
				$('#startTime').html(this_month)
				$('#endTime').html(currentdate)
				break;
			// 本周
			case 'this-k':
...
function thisWeek() {
	...
	if ((day - week) <= 0 && (month - 1) > 0) {
		last_month = (month - 1) < 10 ? '0' + (month - 1) : (month - 1);
		console.log(last_month)
		if (last_month == 2) {
			if (isLeap()) {
				// console.log('2月润年')
				this_week = date.getFullYear() + '-' + last_month + '-' + ((day - week) + 29);
			} else {
				// console.log('2月平年')
				this_week = date.getFullYear() + '-' + last_month + '-' + ((day - week) + 28);
			}
		} else if (last_month < 8) {
			console.log()
			if (last_month % 2 > 0) {
				// console.log('8月份以下的大月')
				this_week = date.getFullYear() + '-' + last_month + '-' + ((day - week) + 31);
			} else {
				// console.log('8月份以下的小月')
				this_week = date.getFullYear() + '-' + last_month + '-' + ((day - week) + 30);
			}
		} else {
			if (last_month % 2 > 0) {
	...
	} else if ((day - week) < 0 && (month - 1) <= 0) {
		// last_year
		this_week = (year - 1) + '-' + 12 + '-' + ((day - week) + 31);
	} else {
		var Day = (day - week) < 10 ? '0' + (day - week) : (day - week);
		// console.log(Day<0)
